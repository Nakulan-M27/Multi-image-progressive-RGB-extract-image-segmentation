...
class Urban3dDataset(Dataset):
    ...
    def __init__(
        ...
        self.images = {f: np.load(os.path.join(self.root, "images", f)) for f in self.filenames}
        self.masks = {f.replace(".npy", "_mask.npy"): np.load(os.path.join(self.root, "masks", f.replace(".npy", "_mask.npy"))) for f in self.filenames}
