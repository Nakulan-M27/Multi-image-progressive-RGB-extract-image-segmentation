...
def build_dataloader(dataset, data_root, boundary_ks, transforms, resize=2048, split=2):
    if dataset == "urban3d":
        ...
        train = CrowdAIDataset(os.path.join(data_root, "AICrowd/train"), boundary_ks, transforms, data_slice=1.0)
        val = CrowdAIDataset(os.path.join(data_root, "AICrowd/val"), boundary_ks, transforms, data_slice=1.0)
