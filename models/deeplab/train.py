...
from models.utils.collate_fn import generate_split_collate_fn, handle_concatenation, handle_normal
...
class Trainer(object):
    def __init__(self, args):
        ...
        resize = 2048
        split = 2
        args.split = split  # Add split to args
    ...
    def validation(self, epoch):
        ...
        state = {
            ...
            'best_miou': np.mean(total_mIOU)
        }
        self.saver.save_checkpoint(state, np.mean(total_loss), np.mean(total_mIOU))

        # select random image and log it to WandB
        if self.args.dataset == "crowdAI":
            # For AICrowd dataset, use handle_normal since images are already in correct size
            filename, image, pred, target = handle_normal(image, pred, target, names)
        else:
